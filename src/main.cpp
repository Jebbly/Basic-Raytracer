...
int main()
{
    ...
    material::ColorMaterial glass{image::Color{1, 1, 1}, 0, 0, 0.9, 200, 1.0, 1.0, 1.52};
    material::ColorMaterial air{image::Color{1, 1, 1}, 0, 0, 0.9, 200, 1.0, 1.0, 1.0};
    geometry::primitive::Plane floor{math::identity(), std::make_shared<material::pattern::CheckerMaterial>(grid)};

    // w.add_object(floor);

    geometry::primitive::Sphere middle{math::identity().translate(-0.5, 1, 0.5), /*Material{image::Color{0.1, 1, 0.5), 0.1, 0.7, 1, 200}*/ std::make_shared<material::pattern::GradientMaterial>(gradient)};
    geometry::primitive::Cone right{math::identity()./*scale(0.3, 0.5, 0.3).*/translate(1.2, 0, -0.5), /*Material{image::Color{0.5, 1, 0.1), 0.1, 0.7, 1, 32}*/ std::make_shared<material::pattern::RingMaterial>(ring), 0, 1, true};
    geometry::primitive::Cylinder left{math::identity().scale(0.33, 0.33, 0.33).translate(-1.5, 0.33, -1.5).rotate_y(constants::PI / 4), /*Material{image::Color{1, 0.8, 0.1), 0.1, 0.7, 1, 64}*/ std::make_shared<material::pattern::StripeMaterial>(stripes), 0, 1, true};
    geometry::Group g{};
    ...
    // Mesh mesh{"astronaut.obj", math::identity().scale(0.05, 0.05, 0.05).rotate_y(constants::PI / 4).translate(0.7, 2.7, 0.2)};

    // Mesh test{"test.obj", math::identity()};
    // w.add_object(test);

    geometry::primitive::Sphere s{math::identity().scale(1.4, 1.4, 1.4).translate(0, 2, 0)};
    geometry::primitive::Cube cy{math::identity().rotate_x(constants::PI / 2).rotate_y(constants::PI / 4).translate(0, 2, 0)};
   ...
   // w.add_object(g);
    ...
    scene::light::PointLight light{image::Color{1, 1, 1}, math::point(-5, 5, -5)};
    scene::light::PointLight light2{image::Color{1, 1, 1}, math::point(10, 10, -10)};
